        ...
        s = "<div class=\"message\" id=\"message_" + this.id_msg + "\">\n" +
                ...
                "<p class=\"content\">" + this.content + "</p>\n" +
                    "<div class=\"message-action\">\n" +
                        "<div class=\"likes\">\n" +
                            "<img id=\"likes_" + this.id_msg + "\" src=\"like.png\" alt=\"like\" onclick=\"likeOrUnlike(" + this.id_msg + ");\"/>\n" +
...
function makeMainPanel(fromId, fromLogin, fromAuthor, query){
    ...
    if (env.fromId >= 0){ // page d'un utilisateur
        ...
        else if (following.includes(env.fromId.toString())){ // l'utilisateur connecté le suit déjà, proposer de ne plus suivre
...
function newCommentResponse(id, resp, text){
    ...
    if (com != undefined && (com.error == undefined)){
        ...
        $("#message_" + id + " .comments-button").text("comments (" + env.msgs[index].comments.length + ")");
...
function likeOrUnlike(id){
    ...
    if (env.msgs[index].likes.includes(env.id_user.toString())){
...
function addLikeResponse(id, resp){
    ...
    if (r.error == undefined){
        ...
        console.log(env.msgs[index].likes); 
        env.msgs[index].likes.push(env.id_user.toString());
        el.text(env.msgs[index].likes.length);
        //console.log(env.msgs[index].likes);
        var bt = $("#likes_" + id);
        bt.replaceWith("<img id=\"likes_" + id + "\" src=\"redlike.png\" alt=\"like\" onclick=\"likeOrUnlike(" + id + ");\"/>\n");
...
function removeLike(id){
    ...
    if (!(env.noConnection)){
       $.ajax({
                ...
                data:"key_user=" + env.key + "&id_message=" + id ,
                datatype:"text",
                success:function(resp){ removeLikeResponse(id, resp);},
                error:function(XHR, textStatus,errorThrown) { alert(textStatus);}
            })
    }
    else{
        removeLikeResponse(id, "{\"key\": \"FARA123\", \"id\": 1, \"login\": \"hugowyb\", \"author\": \"Hugo Wyborska\"}");

    }
}
    
function removeLikeResponse(id,resp){
    console.log(resp);
    var r = JSON.parse(resp, revival);
    var el = $("#message_" + id + " .likes p");
    ...
    if (r.error == undefined){
        ...
        var index_user = env.msgs[index].likes.indexOf(env.id_user.toString());
        //delete(env.msgs[index].likes[index_user]);
        env.msgs[index].likes.splice(index_user, 1);
        el.text(env.msgs[index].likes.length);
        var bt = $("#likes_" + id);
...
function deleteCommentResponse(id, resp){
            ...
            if (msg.id_msg == id_msg){
                ...
                msg.comments.forEach(function (comment){
                    if (comment.id_comment == id){
                        index_comment = env.msgs[index_msg].comments.indexOf(comment);
                    }
                })
            }
        })
        delete(env.msgs[index_msg].comments[index_comment]);
        $("#comment_" + id).remove();
...
function removeFollower(){
    if (!env.noConnection){
        $.ajax({
            ...
            data:"key=" + env.key + "&id_friend=" + env.fromId,
            datatype:"text",
